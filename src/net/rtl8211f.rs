// Supports Realtek RTL8211F

#![no_std]

extern crate alloc;

use crate::cpu::{flush_cache, invalidate_dcache, fence_w};
//use super::mii::*;
use crate::net::mii::*;
...
const PINCTRL_GPIO_BASE: u32 = 0x2000000;
...
pub struct RTL8211F<P: Provider> {
    ...
    recv_ring_phy: &'static mut [dma_desc],

    send_buffers: Vec<usize>,
    send_ring: &'static mut [dma_desc],
    send_ring_phy: &'static mut [dma_desc],
...
impl<P> RTL8211F<P> where P: Provider {
    pub fn new(mac_addr: &[u8; 6]) -> Self {
        ...
        println!("mac addr: {:x}:{:x}:{:x}:{:x}:{:x}:{:x}", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
        ...
        let send_ring_phy = unsafe {
            slice::from_raw_parts_mut(
                send_ring_pa as *mut dma_desc,
                P::PAGE_SIZE / size_of::<dma_desc>(), // 4096/16 = 256 个 dma_desc
                )
        };
        ...
        let recv_ring_phy = unsafe {
            slice::from_raw_parts_mut(
                recv_ring_pa as *mut dma_desc,
                P::PAGE_SIZE / size_of::<dma_desc>(),
                )
        };
        ...
        println!("Set a ring desc buffer for TX");
        ...
        for i in 0..send_ring.len() {
            ...
            if (i + 1) == send_ring.len(){
                send_ring[i].desc3 = &send_ring_phy[0] as *const dma_desc as u32;
            }else{
                send_ring[i].desc3 = &send_ring_phy[i+1] as *const dma_desc as u32;
            }

            send_buffers.push(buffer_page_va);
        }

        println!("Set a ring desc buffer for RX");
        ...
        for i in 0..recv_ring.len() {
            ...
            if (i + 1) == recv_ring.len(){
                recv_ring[i].desc3 = &recv_ring_phy[0] as *const dma_desc as u32;
            }else{
                recv_ring[i].desc3 = &recv_ring_phy[i+1] as *const dma_desc as u32;
            }
        ...
        println!("send_buffers length: {}, recv_buffers length: {}", send_buffers.len(), recv_buffers.len());
    ...
    pub unsafe fn open(&mut self) -> Result<i32, &str> {
        ...
        flush_cache(&self.recv_ring_phy[0] as *const dma_desc as u64, (size_of::<dma_desc>() * self.recv_ring.len()) as u64);
        flush_cache(&self.send_ring_phy[0] as *const dma_desc as u64, (size_of::<dma_desc>() * self.send_ring.len()) as u64);

        // phy_start
        // 注意地址32位对齐
        self.start_rx(&self.recv_ring_phy[0] as *const dma_desc as u32);
        self.start_tx(&self.send_ring_phy[0] as *const dma_desc as u32);
    ...
    pub unsafe fn pinctrl_gpio_set_gmac(&mut self) {
    ...
    pub unsafe fn pinctrl_gpio_reset_gmac_phy(&mut self) {
    ...
    pub unsafe fn set_rx_mode(&mut self) {
        self.set_filter(0x0);
    ...
    pub unsafe fn adjust_link(&mut self) -> Result<i32, &str> {
        let phyaddr = 0;
        let mut link: u32 = 0;
        ...
        loop {
            ...
            if (link == BMSR_LSTATUS) {
                println!("Link is up! status: {:#x}", status);
                break;
            }
        }
    ...
    pub fn geth_recv(&mut self, limit: usize) -> Result<i32, &str> {
        ...
        while rxcount < limit {
            ...
            invalidate_dcache(desc as *const dma_desc as u64, size_of::<dma_desc>() as u64);
            ...
            println!("\nRx frame size {}, status: {:?}", frame_len, status);
            ...
            invalidate_dcache(self.recv_buffers[entry] as u64, frame_len as u64);
            ...
            if status == rx_frame_status::discard_frame as i32 {
                ...
                flush_cache(self.recv_buffers[entry] as u64, 64);
            ...
            // 注意用网络包 skb 的物理地址
            flush_cache(self.recv_buffers[entry] as u64, frame_len as u64);
            ...
            */
            ...
            println!("desc_buf_get_addr: {:#x}, desc_buf_get_len: {}", desc.desc2, desc.desc1 & ((1 << 11) - 1));
        ...
        unsafe {
            println!("\nRX DMA State: {:#x}", read_volatile((self.base + GETH_RX_DMA_STA) as *mut u32));
        }

        if rxcount > 0 {
            println!("######### RX Descriptor DMA: {:#x}", self.recv_ring.as_ptr() as usize);
            println!("RX pointor: dirty: {}, clean: {}", self.rx_dirty, self.rx_clean);
    ...
    pub fn geth_send(&mut self, send_buff: &[u8]) -> Result<i32, &str> {
        ...
        flush_cache(&self.send_ring[desc_count] as *const dma_desc as u64, size_of::<dma_desc>() as u64);
        flush_cache(self.send_buffers[desc_count] as u64, send_buff.len() as u64);

        println!("######### TX Descriptor DMA: {:#x}", self.send_ring.as_ptr() as usize);
        println!("TX pointor: dirty: {}, clean: {}", self.tx_dirty, self.tx_clean);
        println!("[0]: {:#x?} \n[first]: {:#x?} \ndesc: {:#x?}", self.send_ring[0], self.send_ring[first], self.send_ring[desc_count]);


        unsafe{
        println!("TX DMA State: {:#x}", read_volatile((self.base + GETH_TX_DMA_STA) as *mut u32));
        }
    ...
    pub fn rx_refill(&mut self) {
        ...
        while if self.rx_dirty >= (self.rx_clean + 1) {
            ...
            */
            ...
            desc_buf_set(desc, paddr, MAX_BUF_SZ);
            desc_set_own(desc);
            flush_cache(&self.recv_ring[entry] as *const dma_desc as u64, size_of::<dma_desc>() as u64);
    ...
    pub unsafe fn phy_restart_aneg(&mut self) {
    ...
    pub unsafe fn phy_modify(&mut self, regnum: u32, mask: u32, set: u32) -> Result<i32, &str> {
        let phyaddr = 0;
        ...
        */
        ...
        println!("phy_modify, read: {:#x}, set: {:#x}", ret, new);
    ...
    pub unsafe fn power_on(&mut self) {
    ...
    pub unsafe fn clk_enable(&mut self) {
    ...
    pub unsafe fn deassert_emac_reset(&mut self) {
    ...
    pub unsafe fn interrupt_handle(&mut self, irq: u32, dev_id: &u32) -> Result<i32, &str> {
        // int status register
        ...
        if status == tx_dma_irq_status::handle_tx_rx as i32 {
            ...
            //只收一个包
            let work_done = self.geth_recv(1).unwrap();

            //let work_done = self.geth_recv(BUDGET).unwrap();
    ...
    pub unsafe fn int_enable(&mut self) {
        info!("Int enable");
        write_volatile((self.base + GETH_INT_EN) as *mut u32, RX_INT | TX_UNF_INT);
    }

    pub unsafe fn int_disable(&mut self) {
        info!("Int disable");
    ...
    pub unsafe fn dma_init(&mut self) {
        write_volatile((self.base + GETH_BASIC_CTL1) as *mut u32, (8 << 24)); // burst
        self.int_enable();
    }

    pub unsafe fn mac_reset(&mut self) -> Result<i32, &str> {
    ...
    pub unsafe fn mac_init(&mut self, txmode: usize, rxmode: usize) {
        self.dma_init();
    ...
    pub unsafe fn mac_enable(&mut self) {
        let mut value: u32 = read_volatile((self.base + GETH_TX_CTL0) as *mut u32);
        value |= (1 << 31);
        write_volatile((self.base + GETH_TX_CTL0) as *mut u32, value);
    ...
    pub unsafe fn mac_disable(&mut self) {
        let mut value: u32 = read_volatile((self.base + GETH_TX_CTL0) as *mut u32);
        value &= !(1 << 31);
        write_volatile((self.base + GETH_TX_CTL0) as *mut u32, value);
    ...
    pub unsafe fn set_umac(&self, addr: &[u8; 6], index: u32) {
    ...
    pub unsafe fn start_rx(&mut self, rxbase: u32) {
        //rxbase需要32位对齐
    ...
    pub unsafe fn stop_rx(&mut self) {
        let mut value: u32 = read_volatile((self.base + GETH_RX_CTL1) as *mut u32);
        value &= !0x40000000;
        write_volatile((self.base + GETH_RX_CTL1) as *mut u32, value);
    }

    pub unsafe fn start_tx(&mut self, txbase: u32) {
        //txbase需要32位对齐
    ...
    pub unsafe fn stop_tx(&mut self) {
        let mut value: u32 = read_volatile((self.base + GETH_TX_CTL1) as *mut u32);
        value &= !0x40000000;
        write_volatile((self.base + GETH_TX_CTL1) as *mut u32, value);
    }
    ...
    pub unsafe fn pinctrl_gpio_set(&mut self, offset: u32, value: u32) {
        ...
        //regval |= value;
        println!("GPIO offset: {:#x}, read regval: {:#x}", offset, regval);
        write_volatile((self.pinctrl + offset) as *mut u32, value);
    }

    pub unsafe fn hash_filter(&mut self, low: u32, high: u32) {
        println!("RX hash filter low: {:#x}, high: {:#x}", low, high);

        write_volatile((self.base + GETH_RX_HASH0) as *mut u32, high);
        write_volatile((self.base + GETH_RX_HASH1) as *mut u32, low);
    }

    pub unsafe fn set_filter(&mut self, flags: u64) {
        ...
        println!("Set RX frame filter, flags: {:#x}, write value: {:#x}", flags, tmp_flags);
        write_volatile((self.base + GETH_RX_FRM_FLT) as *mut u32, tmp_flags);
    }

    pub unsafe fn set_link_mode(&mut self, duplex: i32, speed: i32) {
        ...
        println!("Set link mode:  duplex {}, speed {}, CTL0: {:#x}", duplex, speed, value);
    }

    pub unsafe fn mac_loopback(&mut self, enable: u32) {
    ...
    pub unsafe fn flow_ctrl(&mut self, duplex: i32, fc: u32, pause: u32) {
        let mut flow: u32 = 0;
        println!("Set flow ctrl: duplex {}, fc {}, pause {}", duplex, fc, pause);
    ...
    pub unsafe fn mdio_read(&mut self, phyaddr: u32, phyreg: u32) -> u32 {
        let mut value: u32 = 0;
        ...
        // println!("mdio_read MDIO DATA: {:#x}", ret);

        ret as u32
    }

    pub unsafe fn mdio_write(&mut self, phyaddr: u32, phyreg: u32, data: u32) {
    ...
    pub unsafe fn mdio_reset(&mut self) {
...
use crate::init::Driver;

impl<P> Driver for RTL8211F<P> where P: Provider + Send + Sync {
    fn try_handle_interrupt(&mut self, irq: Option<u32>, opt: usize) -> bool {
        ...
        if opt == 99 {
            ...
            // Request who-has 192.168.1.127 tell 192.168.1.123
            /*
            let ping_frame: Box<[u8]> = Box::new(
                [0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x3a, 0xc5, 0x31, 0xd5, 0xde, 0x88, 0x08, 0x06, 0x00, 0x01,
                 0x08, 0x00, 0x06, 0x04, 0x00, 0x01, 0x3a, 0xc5, 0x31, 0xd5, 0xde, 0x88, 0xc0, 0xa8, 0x01, 0x7b, //192.168.1.123
                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0xa8, 0x01, 0x7f]); //192.168.1.127
                 */
            let ping_frame: Box<[u8]> = Box::new(
                [0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x3a, 0xc5, 0x31, 0xd5, 0xde, 0x88, 0x08, 0x06, 0x00, 0x01,
                 0x08, 0x00, 0x06, 0x04, 0x00, 0x01, 0x3a, 0xc5, 0x31, 0xd5, 0xde, 0x88, 0xc0, 0xa8, 0x00, 0x7b, //192.168.1.123
                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0xa8, 0x00, 0x69]); //192.168.1.127

            self.geth_send(&ping_frame);
        ...
        info!("geth interrupt handle ...");
        unsafe {
            self.interrupt_handle(irq.unwrap(), &0);
        }
